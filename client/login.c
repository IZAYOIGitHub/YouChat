...
int ShowLogin() //显示登录界面
{
	...
	exitHandler = g_signal_connect(G_OBJECT(window_login), "destroy", G_CALLBACK(gtk_main_quit), NULL); //为信号destroy绑定响应函数
...
static void button_login_clicked() //点击按钮时的响应函数
{
	...
	if (CheckInput(username, password) == 0) //检查输入
	{
		if (client_login((char *)username, (char *)password) == 1)
		{
			g_print("success\n");
			OpenMainPage();
			g_signal_handler_disconnect(window_login, exitHandler);
...
int send_message_tcp(int send_socket_fd, char *buffer)
{ // execute once, send message once
...
int logout(void)
{
	...
	cJSON_AddItemToObject(json, "source", cJSON_CreateString(local_account));
	cJSON_AddItemToObject(json, "destination", cJSON_CreateString("password"));
	cJSON_AddItemToObject(json, "message", cJSON_CreateString("123"));
